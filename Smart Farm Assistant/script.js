...
function initChatbot() {
    ...
    function getResponseFromKnowledgeBase(message) {
        ...
        // Loop through all topics in the farming knowledge base
        for (const category in farmingTopics) {
            ...
            for (const keyword of topicData.keywords) {
                if (query.includes(keyword.toLowerCase())) {
                    const score = calculateRelevanceScore(query, keyword, topicData.keywords);
        ...
        // If we found a match, return a response
        if (bestMatch && highestScore > 0.3) {
            return formatResponse(bestMatch.responses);
        }
        ...
        return "I'm not sure about that specific farming topic. Would you like to know about planting schedules, pest control, or soil management instead?";
    }
